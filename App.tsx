...
type InventoryItem = { id?: string | number; name?: string; itemName?: string; quantity?: number; qty?: number; stock?: number; image?: string };
type MaintenanceEntry = { id?: string | number; title?: string; type?: string; workType?: string; date?: string; createdAt?: string; user?: string; notes?: string; status?: string };
...
function InventoryScreen({ user }: { user: User }) {
  ...
  useEffect(() => { load(); }, []);
  const filtered = useMemo(() => items.filter((item) => String(item.name || item.itemName || '').toLowerCase().includes(query.toLowerCase())), [items, query]);
    ...
    <View style={styles.screen}>
      <FlatList
        ...
        renderItem={({ item }) => {
          ...
          const name = String(item.name || item.itemName || 'صنف بدون اسم');
